...
app.factory('NgTableParams', ['$q', '$log', 'ngTableDefaults', function($q, $log, ngTableDefaults) {
    ...
    var NgTableParams = function(baseParameters, baseSettings) {
         ...
         */
        this.generatePagesArray = function(currentPage, totalItems, pageSize,flag) {
            ...
            if (numPages > 1) {
                ...
                maxPivotPages = Math.round((maxBlocks - 3) / 2);
                ...
                while (i <= maxPage) {
                	if(flag){
                    	if ((i === minPage && i !== 2) || (i === maxPage && i !== numPages - 1 && i !== numPages)) {
                            pages.push({
                                type: 'more',
                                active: false
                            });
        ...
        this.reload = function() {
            ...
            //////////////////////////////////////////////////////////////////
            //FIXME: Hotfix, support loading text
//            self.data = [];
//            settings.$scope.$data = [];
            //////////////////////////////////////////////////////////////////
            ...
            return pData.then(function(data) {
            	/*无数据重置到第一页start*/
            	if(data.length == 0 && self.page() > 1){
            		 if (settings.$scope) {
            			 settings.$scope.params.$params.page = 1;
            		 }
            	}
            	/*无数据重置到第一页end*/
                ...
                var flag = false;
                if("ng-table/pager_noEndPage.html" === settings.$scope.templates.pagination){
                	flag =true;
                }
                if (settings.$scope) settings.$scope.pages = self.generatePagesArray(self.page(), self.total(), self.count(),flag);
        ...
        this.reloadPages = function() {
            ...
            settings.$scope.pages = self.generatePagesArray(self.page(), self.total(), self.count(),flag);
        };
...
function($scope, NgTableParams, $timeout, $parse, $compile, $attrs, $element, ngTableColumn) {
    ...
    this.compileDirectiveTemplates = function (customPaging,freeze) {
        if (!$element.hasClass('ng-table')) {
        	$scope.templates = {
        			header: ($attrs.templateHeader ? $attrs.templateHeader : 'ng-table/header.html'),
        			loading: ($attrs.templateLoading ? $attrs.templateLoading : 'ng-table/loading.html'),
        			pagination: ($attrs.templatePagination ? $attrs.templatePagination : 'ng-table/pager.html')
        	};
        	if(customPaging){
        		$scope.templates.pagination = ($attrs.templatePagination ? $attrs.templatePagination : 'ng-table/pager_noEndPage.html')
        	}
            ...
            var loadingTemplate = angular.element(document.createElement('div')).attr({
                'ng-table-loading': 'params',
                'template-url': 'templates.loading'
            });
            $element.after(loadingTemplate);
        ...
        return {
            ...
            compile: function(element) {
                ...
                return function(scope, element, attrs, controller) {
                    ...
                    controller.compileDirectiveTemplates(attrs.custompaging,attrs.freeze);
                };
            }
        }
    }
    ...
    function($compile) {
        ...
        return {
            ...
            link: function(scope, element, attrs) {
            	scope.pageCount = scope.params.settings().counts[0].toString();
            	var flag = false
            	if("ng-table/pager_noEndPage.html" === scope.params.settings().$scope.templates.pagination){
                	flag =true;
                }
                scope.params.settings().$scope.$on('ngTableAfterReloadData', function() {
                    scope.pages = scope.params.generatePagesArray(scope.params.page(), scope.params.total(), scope.params.count(),flag);
                    scope.pageCount = scope.params.count().toString();
                   /* var $table = $(element).siblings("table");
                    var freeze = $table.attr("freeze") || "";
                    if(freeze == 'true' && $table && $table.FrozenTable){
...
/**
 * @ngdoc directive
 * @name ngTable.directive:ngTableLoading
 * @restrict A
 */
app.directive('ngTableLoading', ['$compile',
    function($compile) {
        'use strict';
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pager_noEndPage.html', '<div class="ng-cloak ng-table-pager" ng-if="params.data.length > 0"><div class="ng-table-counts btn-group pull-right page-count-box"><span>每页显示</span><select ng-model="pageCount" ng-change="params.count(pageCount);"><option ng-repeat="count in params.settings().counts" value="{{count}}" ng-selected="params.count()==count">{{count}}条</option></select> <i class="tb-refresh" title="刷新" ng-click="params.reload();"></i> </div> <ul class="pagination ng-table-pagination"> <li ng-class="{\'disabled\': !page.active && !page.current, \'active\': page.current}" ng-repeat="page in pages" ng-switch="page.type"> <a ng-switch-when="first" ng-click="params.page(page.number)" href="" class="first">首页</a> <a ng-switch-when="first" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="page" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a></li> </ul> </div> ');
}]);
    return app;
}));