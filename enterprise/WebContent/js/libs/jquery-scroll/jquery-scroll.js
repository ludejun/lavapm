;(function($){ 
	$.extend({
		jqueryScroll :function(wrapper,clude_con,type,arrow,isShowSroll){
		var buttons_group;
		var botton_gg=wrapper.find(".bottom_scroll");
		if(botton_gg.length==0){
			if(arrow==true){//arrow 是否显示两端的箭头按钮
				  buttons_group=$('<div class="bottom_scroll">\
		  ...
		  var bottom_scroll = wrapper.find(".bottom_scroll")
		  ...
		  var clude_con = clude_con;//wrapper.find(".content");//主体内容的父级
		  ...
		  if(type=="h"){//水平
				...
				new_h = ph * handle_par.height() / move_content.height();
				if(move_content.height() <= ph){
					//bottom_scroll.remove();
					bottom_scroll.addClass("hide");
					wrapper.parent().removeClass("has-scroll").addClass("has-not-scroll");
					
					if (wrapper[0].removeEventListener){
						wrapper[0].removeEventListener(
							'DOMMouseScroll',
							function (e) {
					...
					wrapper[0].onmousewheel = null;
					
					return false;
				}else{
					wrapper.parent().addClass("has-scroll").removeClass("has-not-scroll");
					bottom_scroll.removeClass("hide");
				}
				//alert(ph+'--'+handle_par.height()+'--'+move_content.height());
				
				//console.dir([clude_con.height(),handle_par.height(),move_content.height(),new_h]);
		  ...
		  handle_c[0].onmouseup = function (e) {//鼠标抬起
			  stopDefault(e);
			  //var e=e||window.event;
			  is_move = 1;
		 ...
		 //点击滚动曹的事件
		  handle_par.click(function (e) {
			  ...
			  if(type=="h"){
				  ...
				  if(new_s_top != undefined){
					  var hg = handle_c.height();
					  if(hg+Math.abs(new_s_top) > ph){
						  //new_s_top = ph - hg;
						  return false;
					  }
					  if(new_m_top != undefined){
		  ...
		  var span_m = 10;
		  var timer;
		  arrow_btn.mousedown(function (e) {
			  stopDefault(e);
			  var objiect = $(this);
			  var flag = false;
			  var stop;
