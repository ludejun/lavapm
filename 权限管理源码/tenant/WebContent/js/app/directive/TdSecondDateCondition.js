define(['app','angularAMD'], function (app,angularAMD) {
    'use strict';
    angularAMD.factory('DataRangeServiceSecond', [ '$rootScope', function($rootScope) {
		...
		return {
			...
			setDateRange : function(d) {
					...
					if(fu.length >1){
						dateRange = d;
						$rootScope.$broadcast('td-second-date-condition.onChange', {
							startDate : se[0],
							endDate : se[1],
							range : d
						});
    ...
    angularAMD.directive('tdSecondDateCondition',['DataRangeServiceSecond','$timeout', function(DataRangeServiceSecond,$timeout) {
    	return {
    		restrict : 'EA',
    		scope:{
    			dateRange:"=",
    		},
    		templateUrl : "js/app/directive/template/td-second-date-condition.html",
    		link: function(scope, elem, attrs) {
    			...
    			scope.setDateRange = function(a,b){
    				if(a && b){
    					if(a == b){
    						scope.displayValueSecond =a;
    					}else{
    						scope.displayValueSecond =a+"~"+b;
    					}
            			DataRangeServiceSecond.setDateRange(a+"~"+b);
    				}else if(a){
    					scope.displayValueSecond =a;
            			DataRangeServiceSecond.setDateRange(a);
    				}else{
    					scope.start=null;
    					scope.end=null;
        				scope.displayValueSecond ="请选择日期";
        				DataRangeServiceSecond.setDateRange(scope.displayValueSecond);
    				}
    			}
    			scope.init = function(){
    				scope.displayValueSecond ="请选择日期";
