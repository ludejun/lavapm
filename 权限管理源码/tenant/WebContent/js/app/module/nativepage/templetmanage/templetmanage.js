define([ 'app','appCommon', 'angularAMD','app/service/ReportService','app/directive/TdHighchar','app/directive/TdHighcharMap','app/directive/TdSecondDateCondition',
         'app/directive/TdDateCondition','app/directive/TdFilterBox','app/directive/TdPanel','app/filter/DateStrSubstring','app/service/EventFlowService',
		...
		return ['$scope', '$location', '$filter', 'NgTableParams','BreakCrumbService',"ReportService" ,"EventFlowService",'$state','$timeout','Upload',
        function($scope, $location, $filter, ngTableParams,BreakCrumbService,reportService,eventFlowService,$state,$timeout,Upload) {
			...
			$scope.askTempletmanageGroup = function(data){
			    appConfig.Templetmanage=data!=null?data:{};
			    $("#createTempletmanage").click();
				
			}
			
			$scope.askTempletmanageView = function(data){
			    appConfig.Templetmanage=data!=null?data:{};
			    $("#viewTempletmanage").click();
			...
			$scope.stopEvent = function(event){
        		$.Pop.confirms('确认删除'+event.t_name+'？',function(){
        			var params={
    						rids:event.rids+"",
    						tids : event.tids+"",
    				};
        			reportService.getSaveDeleByParams(params).then(function (data){	
