define([ 'app','appCommon', 'angularAMD','app/service/ReportService','app/directive/TdSecondDateCondition',
        ...
        function($scope, $location, $filter,BreakCrumbService,reportService,$state,$stateParams) {
			...
			$scope.switchPageApp= function(flg){
				...
				}else if(appflg && flg == 1){
					$scope.switchApp($scope.authAppListTop11[$scope.authAppListTop11.length-1]);
					$scope.switchPage($scope.authAppListTop11[$scope.authAppListTop11.length-1].appCode);
				}

			}
			if($scope.templet.t_name!=''&&$scope.templet.t_name!=null){
				$scope.title="修改权限模板"
			}else{
				$scope.title="创建权限模板"
			}
			...
			$scope.saveUserSegmentation = function(){
				 ...
				 if(!$scope.templet.t_name){
					 $.Pop.alerts("请输入权限模板名称！");
					 return ;
				 }
				 var params = {
					      roleName:$scope.templet.t_name,
					      roleDesc:$scope.templet.t_desc,
					      tids:$scope.templet.tids,
					      rids:$scope.templet.rids,
					      type:$scope.templet.type,
			  	};
				 var _repotresList=new Array();
				 angular.forEach($scope.allList,function(item,index){
					 
					 if(!item.value.list||!item.value.list) return;
					 angular.forEach(item.value.list,function(item1,index){
						 var templist=[];
						 $scope.savepush(item1.children,templist);
						 if(templist.length>0){
							 templist.push(item1.rid)
						 }
						 var temp={
							key:item.key,
