define([ 'angularAMD', 'app/controllers/tag/TagBuilder', 'app/controllers/tag/TagDefinitionController','css!/enterprise/css/libs/zTreeStyle/zTreeStyle','css!/enterprise/css/app/searchTagator/searchTagator','css!../../../../css/app/tag/tag','jquery.ztree.core-3.5','jquery.ztree.excheck-3.5','jquery.ztree.exedit-3.5', 'app/services/tag/TagService', 'app/services/tag/TagCategoryService', 'app/services/admin/DicService', 'app/services/common/CommonService', 'app/controllers/common/searchTagator', 'app/filters/common/CommonFilter' ], function(angularAMD, TagBuilder, TagDefinitionController) {
	'use strict';
	...
	return [ '$scope', '$rootScope', '$stateParams', 'TagService', 'TagCategoryService', 'DicService', 'CommonService', '$state', function($scope, $rootScope, $stateParams, TagService, TagCategoryService, DicService, CommonService, $state) {
		...
		$scope.tagDefinitionController = null;
		...
		$scope.createTag = function(tag,formHorizontal) {
			TagService.create(tag).then(function(tag) {
				$.Pop.alerts($scope.nlsRoot.addTagSuccess, "/tag/tags");
				$.layerLoading.hide();
				// $state.go('tags');
				// $state.reload();
			}, function(response) {
				...
				if (response.data && !response.data.success) {
					if(response.data.errorCode && response.data.errorCode == '8001'){
						$.Pop.alerts(response.data.msg, "/tag/tags");
		...
		$scope.saveTag = function(tag,formHorizontal,isNeedCalculate) {
			...
			var respone = this.tagDefinitionController.buildDefinition();
			if(respone.success && respone.db && respone.db.definition){
				...
				var buildNexus = this.tagDefinitionController.buildNexus(definition);
				tag.tagCategoryIds = $scope.tagCategoryObj.ids;
				tag.description = buildNexus;
		...
		$scope.changeTouchPointType = function(tag) {
			this.tagDefinitionController.model.deviceType = tag.touchPointType;
		...
		$scope.initDicByName = function() {
			$scope.tagDefinitionController = new TagDefinitionController();
		...
		$scope.initDefinitionHtml = function(isReset) {
			var that = this;
			this.tagDefinitionController.model.deviceType = $scope.tag.touchPointType;
			...
			if (isReset) {
				var html = this.tagDefinitionController.initHtml();
				$tag_relations_panel.html(html);
			} else {
				if ($scope.tagId) {// 编辑
					var html = this.tagDefinitionController.buildTagHtmlByTagDefinitionData($scope.tag.definition,tableTypes);
					$tag_relations_panel.html(html);
					if($tag_relations_panel.find(".relation-item-attributeTable").length == 0 && !$scope.isViewRule){
						var $btn_add_relation = $tag_relations_panel.find(".btn-add-relation");
						this.tagDefinitionController.createRelationHtml($btn_add_relation,'attributeTable');
					}
					
					$tag_relations_panel.find(".attributeTable .attributes").change();
					$tag_relations_panel.find(".behaviorTable .attributes").each(function(){
						var $attributes = $(this);
					...
					this.tagDefinitionController.checkIsShowAddGroupComboBtn();
					this.tagDefinitionController.checkIsShowAddRelationBtn();
					
					$(".behavior-attributes").each(function(){
						var $behavior_attributes = $(this);
						that.tagDefinitionController.isShowBehaviorAttributeLabelBtn($behavior_attributes);
					});
				}
			}
		}
		...
		$scope.checkDefinitionIsEqual = function(tag,formHorizontal){
			...
			if(respone.success && respone.db && respone.db.definition){
				...
				$scope.tagJsonDefinitionOld = this.tagDefinitionController.model.definition;
		...
		$scope.builderTagJson = function() {
			...
			var indice = db.buildIndice("", "app_tdid", "MetaAccount", "app_tdid_account");
			var query = db.buildMustQueryWithQualifier(db.buildTermQualifier("app_key", db.buildEqOperatorValue("t1")));
			db.setIndice(condition1, indice);
			db.setQuery(condition1, query);

			var condition2 = "condition2";
			indice = db.buildIndice("", "app_tdid", "MetaBehavior", "app_tdid_session");
			query = db.buildMustQueryWithQualifier(db.buildRangeQualifier("platformCount", db.buildRangeOperatorValue("gte", 100, "lte", 200)));
			var subQuery = db.buildSubQueryWithoutQuery("platformCount", db.buildCountAggregation("platform_id"));
			db.setIndice(condition2, indice);
			db.setQuery(condition2, query);
			db.setSubQuery(condition2, subQuery);

			db.setFilter(db.buildNotFilter(db.buildAndFilterWithConditions(new Array(condition1, condition2))));

			var isValid = db.validate();
			console.dir([ "isValid", isValid ]);

			var jsonStr = db.toString();
			console.dir([ "def", jsonStr ]);
			$scope.tagJson = jsonStr;*/
		}
