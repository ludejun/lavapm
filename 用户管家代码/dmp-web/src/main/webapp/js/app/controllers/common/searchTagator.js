(function($) {
	$.tagator = function (element, options) {
		...
		plugin.init = function () {
			...
			if(plugin.settings.minWidthClass){
				$(input_element).addClass(plugin.settings.minWidthClass);
			}
			
			if(plugin.settings.placeHolder){
				$(input_element).attr('placeholder', plugin.settings.placeHolder);
			}else{
				$(input_element).attr('placeholder', '请输入关键字');
			}
			...
			$(input_element).bind('blur', function (e) {
				var $elem = $(this);
				var $tagator = $elem.parentsUntil(".tagator").parent();
				e.preventDefault();
				e.stopPropagation();
				if($tagator.length != 1){
					hideOptions();
		...
		var refreshTags = function () {
			...
			$.each(tags, function (key, value) {
				if (value !== '') {
					...
					$(tag_element).html(value).attr({"title":value});
					// remove button
		...
		var addTag = function (text) {
			var text = $.trim(text);
			if(!plugin.settings.allowCustom){//是否允许自定义内容，不允许自定义内容
				var isInDatas = false;
				$.each(plugin.settings.autocomplete, function (key, value) {
					//if($.trim(value.toUpperCase()) == $.trim(text.toUpperCase())){
					if($.trim(value) == $.trim(text)){
						isInDatas = true;
						text = value;
					}
				});
				if (!hasTag(text) && isInDatas) {
