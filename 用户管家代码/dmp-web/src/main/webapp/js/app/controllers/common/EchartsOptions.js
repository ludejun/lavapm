define([],function(){
var EchartsOptions = {
	_formatNumberToDecimal:function(s, n) {
			var decimalNum = '';
			n = n > 0 && n <= 20 ? n : 2;
			...
			decimalNum = t.split("").reverse().join("") + "." + r;
			
			return decimalNum;
			//return t.split("").reverse().join("");
		},
	_buildChinaMapPointCityData:function(data){
		var result = {};
		for(var i = 0;i < data.length;i++){
			var pointAttr = [];
	    	var g = [];
	    	if(data[i].g){
	    		pointAttr = data[i].g.split("|");
	    		if(pointAttr[0]){
	    			g = pointAttr[0].split(",");
	        		...
	        		if(data[i].n && data[i].n.indexOf("西双版纳") != -1){
	        			result['西双版纳傣族自治州'] = g;
	        		}else if(data[i].n && data[i].n.indexOf("玉树") != -1){
	        			result['玉树藏族自治州'] = g;
	        		}
	    		}
	    	}
	...
	_buildChinaMapPointProvincesData:function(data){
		var result = {};
		for(var i = 0;i < data.length;i++){
			var resultItem = {};
			var provinceDatas = {};
	    	...
	    	var g = [];
	    	if(data[i].g){
	    		pointArr = data[i].g.split("|");
	    		if(pointArr[0]){
	    			g = pointArr[0].split(",");
    				provinceDatas[data[i].n] = g;
    				provinceDatas[data[i].n + '省'] = g;
	    		}
	    	}
	    	var cities = data[i].cities;
	    	cityDatas = EchartsOptions._buildChinaMapPointCityData(cities);
	    	resultItem = $.extend({}, provinceDatas,cityDatas);
	    	result = $.extend({}, result,resultItem);
	    }
		return result;
	},
	_buildChinaMapPointOtherData:function(data){
		var result = {};
		for(var i = 0;i < data.length;i++){
			var pointAttr = [];
	    	var g = [];
    ...
    var others = cityData.other;
    var jsonStr = {};
    
    var municipalitiesJsonStr = EchartsOptions._buildChinaMapPointCityData(municipalities);
    var provincesJsonStr = EchartsOptions._buildChinaMapPointProvincesData(provinces);
    var otherJsonStr = EchartsOptions._buildChinaMapPointOtherData(others);

    jsonStr = $.extend({}, municipalitiesJsonStr,provincesJsonStr,otherJsonStr);
    ...
    /*var jsonStr={
      ...
      var mapData=[];
      var mapGeoCoord={}
      var mapMax = Math.max.apply({},par.value);
      var mapMin = Math.min.apply({},par.value);
      for(var i=0; i<par.key.length; i++){
    	  //if(jsonStr[par.key[i]]){
    		  mapData.push({name:par.key[i],value:par.value[i],rate:par.rate[i]});
    		  mapGeoCoord[par.key[i]]=jsonStr[par.key[i]];
    	  //}
      }
      option = {
          tooltip : {
    ...
    chinaMpaRankingHtml :function(par,num){//name: "武汉",rate: 0.20040485829959515,value: 99
		var i = 1,d = par,html = "";
		for(k in d){
			if(i <= num){
				if(d[k].rate == 'NaN'){
				...
				var per = EchartsOptions._formatNumberToDecimal(parseFloat(d[k].rate)*100,1);
				...
				html += '<span title="'+d[k].name+'">'+d[k].name+'</span><b>'+per+'%</b>';
				html += '</div></li>';
				i++;
			}
		}
	...
	chinaMapRankingHtml:function(par,num){
		var i = 1,d = par,html = '<dt class="clrfix">省份分布Top10</dt>';
		if(d && d.length > 0){
			for(k in d){
				if(i <= num){
					if(d[k].rate == 'NaN'){
					...
					html += '<dd class="clrfix">';
					html += '<span class="number">'+i+'、</span>';
					html += '<span class="text">'+d[k].name+'</span>';
					html += '<div class="progress-box">';
					html += '<div class="progress-inner progress-inner'+ i+'" style="width:'+per+'%;"></div>';
					html += '</div>';
					html += '<span class="percent">'+per+'%</span>';
					html += '</dd>';
					i++;
				}
			}
	...
	chinaMapProvice:function(par){
		var max = 0;
		if(par && par.data && par.data.length > 0){
			if(par.data[0].value){
				max = par.data[0].value;
			}
		}
		var option = {
		    title: {
		...
		option = {
		    ...
		    tooltip : {
		        ...
		        formatter: function(a){ 
		        	if(a[1].indexOf("-") == -1){
		        		res = a[0]+'：'+a[1] + '<br/>关联度：' + a[2]; 
		        	}else{
		        		var nameArr = a[1].split("-");
		        		for (var i = 0; i < par.nodes.length; i++) { 
			        		if($.trim(nameArr[1]) == par.nodes[i].name){ 
			        			//res +=  '</br>关联度：' + par.nodes[i].value + '</br>'+'{b}'; 
			        			res = a[0]+'：'+a[1] + '<br/>关联度：' + par.nodes[i].value;
			        		}
		        		}
		        		...
		        		for (var i = 0; i < nodes.length; i++) { 
			        		if($.trim(nameArr[1]) == nodes[i].name){ 
			        			//res +=  '</br>关联度：' + par.nodes[i].value + '</br>'+'{b}'; 
			        			res = a[0]+'：'+a[1] + '<br/>关联度：' + nodes[i].value;
			        		}
		        		}*/
		        		/*mock tooltip*/
		        	}
		...
		option = {
		    ...
		    tooltip : {
		        ...
		        formatter: function(a){ 
		        	console.dir(['a',a]);
		        	var res = "";
		        	if(a && a[5] && a[5].source && a[5].target){
		        		res = a[0] + '：“' + a[5].source + '”，与“' + a[5].target +'”<br/>关联度：'+a[5].value;
		        	}else if(a && a[5] && a[5].name){
		        		res = a[0] + '：“' + a[5].name + '”';//<br/>关联度：' + a[5].value;
		        	}
		        	return res; 
		        }
		    },
	...
	tagCategoryPie:function(par,isShowLegend,isShowLine){//标签类别子类别人群占比
		isShowLegend = isShowLegend || false;
		isShowLine = isShowLine || false;
		/*var option = {
		    tooltip: {
		        trigger: 'item',
